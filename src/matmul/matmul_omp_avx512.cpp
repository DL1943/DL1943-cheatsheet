...
const int TILE_SIZE = 16; // Tune based on cache and thread performance

void matmul_omp_avx512(const std::vector<std::vector<float>> &A,
            ...
            for (int k = 0; k < K; k += TILE_SIZE) {
                ...
                for (int ii = i; ii < std::min(i + TILE_SIZE, M); ii++) {
                    for (int kk = k; kk < std::min(k + TILE_SIZE, K); kk++) {
                        ...
                        // Load current A block into SIMD register
