...
void matmul_prefetch(const std::vector<std::vector<float>> &A,
    ...
    for (int i = 0; i < M; i += TILE_SIZE) {
        for (int j = 0; j < N; j += TILE_SIZE) {
            for (int k = 0; k < K; k += TILE_SIZE) {
                ...
                // Prefetch next tile of A and B into cache
                if (k + TILE_SIZE < K) { // Ensure within bounds
                    _mm_prefetch((char*)&A[i][k + TILE_SIZE], _MM_HINT_T0);
                    _mm_prefetch((char*)&B[k + TILE_SIZE][j], _MM_HINT_T0);
                }
