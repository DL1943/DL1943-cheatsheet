...
const int TILE_SIZE = 16; // Tune this for better performance

void matmul_tiled(const std::vector<std::vector<float>> &A,
    ...
    for (int i = 0; i < M; i += TILE_SIZE) {
        for (int j = 0; j < N; j += TILE_SIZE) {
            for (int k = 0; k < K; k += TILE_SIZE) {
                ...
                // Process TILE_SIZE × TILE_SIZE block at a time
